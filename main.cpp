#define USE_SOFTMAX //If this is enabled softmax instead of sigmoid used at last laystructureser
...
int get_MNIST_file_size(void)
{
    ...
    if (fp2 == NULL)
    {
        ...
        if(answer_character == 'Y' || answer_character == 'y')
        {
            ...
            sprintf(filename, "train-images-idx3-ubyte.gz");
            check_file_exist(filename);
            sprintf(filename, "train-labels-idx1-ubyte.gz");
            check_file_exist(filename);
            sprintf(filename, "t10k-images-idx3-ubyte.gz");
            check_file_exist(filename);
            sprintf(filename, "t10k-labels-idx1-ubyte.gz");
            check_file_exist(filename);
...
int main()
{
    ...
    printf("================================================================================================\n");
    printf("Important Notice: Finetune option command <I> don't working, feature kernels backpropagation is wrong\n");
    printf("Need 180 deg turn around the feature and others when backprop the features in fine tune mode. The finetune feature need to change implementation.\n");
    printf("Don NOT use command <I> Wrong backprop ! TOGGLE Fine tune Feature L2 from fc backprop (will lock fc weight updates) full_conn_backprop must be ON \n");
    printf("Use instead all other commands when trainging\n");
    printf("================================================================================================\n");
